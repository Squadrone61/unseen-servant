...
- **CharacterData split:** `builder` (full BuilderState snapshot for lossless edit — config only: species/class/abilities/feats/spells/identity, NO inventory/currency) + `static` (slim seed: name/species/classes/abilities/languages/spells/features + permanent `effects: EffectBundle[]`) + `dynamic` (HP, spell slots, conditions, activeEffects, **inventory, currency** — the single source of truth for gear, owned by our system). Derived stats (AC, max HP, skills, saves, senses, spellcasting, proficiencies, class resources, advantages) are computed on demand via resolver accessors in `shared/character/resolve.ts` (`getAC`, `getHP`, `getSkills`, ...). Builder field stripped from DM tools via `get_character`. `buildCharacter(state, starting?: { inventory?, currency? })` is the single entry point; starting inventory/currency seed `dynamic.inventory` / `dynamic.currency` on creation. In the web builder, the Equipment + Details steps write to a sibling store (`BuilderContext.equipment`) that's seeded from `dynamic.*` on edit-mode load and becomes the new `dynamic.*` on save. `CharacterIdentifiers` no longer exists.
