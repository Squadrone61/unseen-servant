...
- **CharacterData split:** `builder` (full BuilderState snapshot for lossless edit) + `static` (slim seed: name/species/classes/abilities/languages/spells/features + permanent `effects: EffectBundle[]`) + `dynamic` (HP, spell slots, conditions, activeEffects — owned by our system). Derived stats (AC, max HP, skills, saves, senses, spellcasting, proficiencies, class resources, advantages) are computed on demand via resolver accessors in `shared/character/resolve.ts` (`getAC`, `getHP`, `getSkills`, ...). Builder field stripped from DM tools via `get_character`. `buildCharacter(state: BuilderState)` is the single entry point; `CharacterIdentifiers` no longer exists.
