...
- `tools/dnd-tools.ts` — roll_dice (checkType always required; with player = interactive player roll, without = DM server-side roll)
...
| `roll_dice`               | Roll dice — checkType always required. With player = interactive roll, without = DM server-side roll. 'custom'/'damage' need notation, others are d20 checks with advantage/disadvantage support. |
