...
| --------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Starting combat | `/combat-prep`           | Looks up every monster, validates difficulty, **calls `update_battle_map` itself**, **calls `save_encounter_bundle` itself** — returns SHORT ENCOUNTER SUMMARY with the **bundle slug**, combatant roster, and opening hook | Call `start_combat` with `combatants` from the summary AND `encounter_bundle_slug: "<slug>"` from the summary → narrate opening. **Do NOT re-call `update_battle_map` or `save_encounter_bundle` — the specialist owns both.** |
| NPC/enemy turn  | `/combat-turn <name>`    | Loads the bundle once, picks a tactic, rolls, returns TURN PLAN. Skips per-ability `lookup_rule` because the bundle is pre-resolved.                                                                                        | Apply MUTATIONS in order, narrate NARRATIVE with entity tags, `advance_turn`                                                                                                                                                   |
...
**Map + bundle ownership**: the specialist that designed the tactical space persists it. Encounter-designer owns `update_battle_map` AND `save_encounter_bundle` — the bundle is the contract that makes per-turn combat fast (combat-resolver reads pre-resolved monster stats once instead of looking up each ability every turn). Your job on combat start is to forward the bundle slug into `start_combat` so the resolver finds it; the staging artifacts themselves are the specialist's deliverable.
