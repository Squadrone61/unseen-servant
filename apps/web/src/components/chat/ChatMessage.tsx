...
function TTSButton({ text }: { text: string }) {
  const { speak, stop, isSpeaking, volume, setVolume } = useTTS();
  ...
  return (
    <div
      ...
      onMouseLeave={() => !isSpeaking && setShowSlider(false)}
    >
      <button
        onClick={() => (isSpeaking ? stop() : speak(text))}
        className="p-1 rounded hover:bg-purple-800/40 text-purple-400 hover:text-purple-300 transition-colors"
