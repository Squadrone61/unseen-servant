/**
 * Expression Evaluator — Value Notation Language
 *
 * Evaluates value expressions used in the effect system. Supports:
 *   - Literals: 5, -2, 10
 *   - Ability modifiers: str, dex, con, int, wis, cha  → Math.floor((score - 10) / 2)
 *   - Context atoms: prof, lvl, clvl
 *   - Arithmetic: +, -, *  (standard precedence: * before +/-)
 *   - Functions: min(a, b), max(a, b), table(L:V, L:V, ...)
 *
 * Usage:
 *   evaluateExpression("10 + dex + con", ctx)  // 17
 *   evaluateExpression("table(1:2, 9:3, 16:4)", ctx)  // 3 at clvl 12
 */

import type { ResolveContext } from "../types/effects";

export type { ResolveContext };

// ---------------------------------------------------------------------------
// Tokeniser
// ---------------------------------------------------------------------------

type TokenKind =
  | "number"
  | "ident"
  | "plus"
  | "minus"
  | "star"
  | "lparen"
  | "rparen"
  | "comma"
  | "colon"
  | "eof";

interface Token {
  kind: TokenKind;
  value: string;
}

function tokenize(src: string): Token[] {
  const tokens: Token[] = [];
  let i = 0;

  while (i < src.length) {
    const ch = src[i];

    // Whitespace
    if (ch === " " || ch === "\t" || ch === "\r" || ch === "\n") {
      i++;
      continue;
    }

    // Number literal (digits only — minus sign handled as unary operator)
    if (ch >= "0" && ch <= "9") {
      let num = "";
      while (i < src.length && src[i] >= "0" && src[i] <= "9") {
        num += src[i++];
      }
      tokens.push({ kind: "number", value: num });
      continue;
    }

    // Identifier: str, dex, con, int, wis, cha, prof, lvl, clvl, min, max, table
    if ((ch >= "a" && ch <= "z") || (ch >= "A" && ch <= "Z") || ch === "_") {
      let ident = "";
      while (
        i < src.length &&
        ((src[i] >= "a" && src[i] <= "z") ||
          (src[i] >= "A" && src[i] <= "Z") ||
          (src[i] >= "0" && src[i] <= "9") ||
          src[i] === "_")
      ) {
        ident += src[i++];
      }
      tokens.push({ kind: "ident", value: ident });
      continue;
    }

    // Single-character tokens
    if (ch === "+") {
      tokens.push({ kind: "plus", value: "+" });
      i++;
      continue;
    }
    if (ch === "-") {
      tokens.push({ kind: "minus", value: "-" });
      i++;
      continue;
    }
    if (ch === "*") {
      tokens.push({ kind: "star", value: "*" });
      i++;
      continue;
    }
    if (ch === "(") {
      tokens.push({ kind: "lparen", value: "(" });
      i++;
      continue;
    }
    if (ch === ")") {
      tokens.push({ kind: "rparen", value: ")" });
      i++;
      continue;
    }
    if (ch === ",") {
      tokens.push({ kind: "comma", value: "," });
      i++;
      continue;
    }
    if (ch === ":") {
      tokens.push({ kind: "colon", value: ":" });
      i++;
      continue;
    }

    throw new Error(`Unexpected character '${ch}' in expression`);
  }

  tokens.push({ kind: "eof", value: "" });
  return tokens;
}

// ---------------------------------------------------------------------------
// Recursive Descent Parser + Evaluator
// ---------------------------------------------------------------------------

function abilityModifier(score: number): number {
  return Math.floor((score - 10) / 2);
}

class Parser {
  private tokens: Token[];
  private pos: number;
  private ctx: ResolveContext;

  constructor(tokens: Token[], ctx: ResolveContext) {
    this.tokens = tokens;
    this.pos = 0;
    this.ctx = ctx;
  }

  private peek(): Token {
    return this.tokens[this.pos];
  }

  private consume(): Token {
    return this.tokens[this.pos++];
  }

  private expect(kind: TokenKind): Token {
    const tok = this.consume();
    if (tok.kind !== kind) {
      throw new Error(`Expected token '${kind}' but got '${tok.kind}' ('${tok.value}')`);
    }
    return tok;
  }

  /** Top-level entry point */
  parseExpr(): number {
    return this.parseAddSub();
  }

  /** additive: term (('+' | '-') term)* */
  private parseAddSub(): number {
    let left = this.parseMul();

    while (this.peek().kind === "plus" || this.peek().kind === "minus") {
      const op = this.consume();
      const right = this.parseMul();
      if (op.kind === "plus") {
        left += right;
      } else {
        left -= right;
      }
    }

    return left;
  }

  /** multiplicative: unary ('*' unary)* */
  private parseMul(): number {
    let left = this.parseUnary();

    while (this.peek().kind === "star") {
      this.consume();
      const right = this.parseUnary();
      left *= right;
    }

    return left;
  }

  /** unary: '-' primary | primary */
  private parseUnary(): number {
    if (this.peek().kind === "minus") {
      this.consume();
      return -this.parsePrimary();
    }
    return this.parsePrimary();
  }

  /** primary: number | ident | ident '(' args ')' | '(' expr ')' */
  private parsePrimary(): number {
    const tok = this.peek();

    if (tok.kind === "number") {
      this.consume();
      return parseInt(tok.value, 10);
    }

    if (tok.kind === "lparen") {
      this.consume();
      const val = this.parseExpr();
      this.expect("rparen");
      return val;
    }

    if (tok.kind === "ident") {
      this.consume();
      const name = tok.value.toLowerCase();

      // Function calls
      if (this.peek().kind === "lparen") {
        this.consume(); // consume '('
        if (name === "min") {
          const a = this.parseExpr();
          this.expect("comma");
          const b = this.parseExpr();
          this.expect("rparen");
          return Math.min(a, b);
        }
        if (name === "max") {
          const a = this.parseExpr();
          this.expect("comma");
          const b = this.parseExpr();
          this.expect("rparen");
          return Math.max(a, b);
        }
        if (name === "table") {
          return this.parseTable();
        }
        throw new Error(`Unknown function '${name}'`);
      }

      // Atoms
      return this.resolveAtom(name);
    }

    throw new Error(`Unexpected token '${tok.kind}' ('${tok.value}') in expression`);
  }

  /**
   * table(L:V, L:V, ...) — already consumed 'table' and '('.
   * Uses clvl. Finds highest entry where L ≤ clvl.
   * Returns 0 if no entry applies.
   */
  private parseTable(): number {
    const entries: Array<{ level: number; value: number }> = [];

    // Parse zero or more L:V pairs separated by commas
    while (this.peek().kind !== "rparen" && this.peek().kind !== "eof") {
      // Level key — may be negative (unlikely but handle it)
      let levelNeg = false;
      if (this.peek().kind === "minus") {
        this.consume();
        levelNeg = true;
      }
      const levelTok = this.expect("number");
      const level = levelNeg ? -parseInt(levelTok.value, 10) : parseInt(levelTok.value, 10);

      this.expect("colon");

      // Value — may be negative
      let valueNeg = false;
      if (this.peek().kind === "minus") {
        this.consume();
        valueNeg = true;
      }
      const valueTok = this.expect("number");
      const value = valueNeg ? -parseInt(valueTok.value, 10) : parseInt(valueTok.value, 10);

      entries.push({ level, value });

      if (this.peek().kind === "comma") {
        this.consume();
      }
    }

    this.expect("rparen");

    const clvl = this.ctx.classLevel ?? this.ctx.totalLevel;

    // Find highest entry where entry.level ≤ clvl
    let result = 0;
    for (const entry of entries) {
      if (entry.level <= clvl) {
        result = entry.value;
      }
    }
    return result;
  }

  private resolveAtom(name: string): number {
    const { abilities, totalLevel, classLevel, proficiencyBonus, stackCount } = this.ctx;

    switch (name) {
      case "str":
        return abilityModifier(abilities.strength);
      case "dex":
        return abilityModifier(abilities.dexterity);
      case "con":
        return abilityModifier(abilities.constitution);
      case "int":
        return abilityModifier(abilities.intelligence);
      case "wis":
        return abilityModifier(abilities.wisdom);
      case "cha":
        return abilityModifier(abilities.charisma);
      case "prof":
        return proficiencyBonus;
      case "lvl":
        return totalLevel;
      case "clvl":
        return classLevel ?? totalLevel;
      case "stacks":
        return stackCount ?? 1;
      default:
        throw new Error(`Unknown identifier '${name}' in expression`);
    }
  }
}

// ---------------------------------------------------------------------------
// Public API
// ---------------------------------------------------------------------------

/**
 * Evaluate a value expression against a resolve context.
 *
 * @param expr  A number (returned as-is) or a string expression like "10 + dex + con".
 * @param ctx   The character context to resolve atoms against.
 * @returns     The evaluated numeric result.
 */
export function evaluateExpression(expr: number | string, ctx: ResolveContext): number {
  if (typeof expr === "number") return expr;

  const tokens = tokenize(expr);
  const parser = new Parser(tokens, ctx);
  const result = parser.parseExpr();

  if (parser["peek"]().kind !== "eof") {
    throw new Error(`Unexpected trailing tokens in expression: '${expr}'`);
  }

  return result;
}

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------

export function runTests(): void {
  const fullCtx: ResolveContext = {
    abilities: {
      strength: 10,
      dexterity: 16, // mod +3
      constitution: 18, // mod +4
      intelligence: 12, // mod +1
      wisdom: 16, // mod +3
      charisma: 8, // mod -1
    },
    totalLevel: 7,
    classLevel: 12,
    proficiencyBonus: 3,
  };

  type TestCase = [expr: number | string, ctx: ResolveContext, expected: number, label: string];

  const tests: TestCase[] = [
    // Literal number passthrough
    [5, fullCtx, 5, "literal number 5"],
    [-2, fullCtx, -2, "literal number -2"],

    // Ability modifiers + addition
    [
      "10 + dex + con",
      { ...fullCtx, abilities: { ...fullCtx.abilities, dexterity: 16, constitution: 18 } },
      17, // 10 + 3 + 4
      "10 + dex + con",
    ],

    // lvl multiplication
    ["2 * lvl", { ...fullCtx, totalLevel: 7 }, 14, "2 * lvl"],

    // prof atom
    ["prof", { ...fullCtx, proficiencyBonus: 3 }, 3, "prof"],

    // table at clvl 12 → 3
    ["table(1:2, 9:3, 16:4)", { ...fullCtx, classLevel: 12 }, 3, "table at clvl 12"],

    // table at clvl 1 → 2
    ["table(1:2, 9:3, 16:4)", { ...fullCtx, classLevel: 1 }, 2, "table at clvl 1"],

    // table at clvl 16 → 4
    ["table(1:2, 9:3, 16:4)", { ...fullCtx, classLevel: 16 }, 4, "table at clvl 16"],

    // max(cha, 1) — CHA 8 → mod -1, max(-1, 1) → 1
    [
      "max(cha, 1)",
      { ...fullCtx, abilities: { ...fullCtx.abilities, charisma: 8 } },
      1,
      "max(cha, 1) with CHA 8",
    ],

    // max(cha, 1) — CHA 20 → mod +5, max(5, 1) → 5
    [
      "max(cha, 1)",
      { ...fullCtx, abilities: { ...fullCtx.abilities, charisma: 20 } },
      5,
      "max(cha, 1) with CHA 20",
    ],

    // min(dex, 2) — DEX 16 → mod +3, min(3, 2) → 2
    [
      "min(dex, 2)",
      { ...fullCtx, abilities: { ...fullCtx.abilities, dexterity: 16 } },
      2,
      "min(dex, 2) with DEX 16",
    ],

    // min(dex, 2) — DEX 12 → mod +1, min(1, 2) → 1
    [
      "min(dex, 2)",
      { ...fullCtx, abilities: { ...fullCtx.abilities, dexterity: 12 } },
      1,
      "min(dex, 2) with DEX 12",
    ],

    // 14 + min(dex, 2) — DEX 16 → min(3, 2) = 2, total 16
    [
      "14 + min(dex, 2)",
      { ...fullCtx, abilities: { ...fullCtx.abilities, dexterity: 16 } },
      16,
      "14 + min(dex, 2) with DEX 16",
    ],

    // 8 + prof + wis — prof 3, WIS 16 → mod +3, total 14
    [
      "8 + prof + wis",
      { ...fullCtx, proficiencyBonus: 3, abilities: { ...fullCtx.abilities, wisdom: 16 } },
      14,
      "8 + prof + wis",
    ],

    // table with no matching entry (all levels > clvl) → 0
    ["table(2:10, 6:15, 10:20)", { ...fullCtx, classLevel: 1 }, 0, "table with no matching entry"],

    // clvl falls back to totalLevel when classLevel is undefined
    [
      "table(1:2, 9:3, 16:4)",
      { ...fullCtx, classLevel: undefined, totalLevel: 9 },
      3,
      "table clvl fallback to totalLevel",
    ],

    // Negative literal in expression
    ["-2", fullCtx, -2, "negative literal string"],

    // Multiplication precedence: 2 + 3 * 4 = 14
    ["2 + 3 * 4", fullCtx, 14, "multiplication precedence"],

    // Parentheses override precedence: (2 + 3) * 4 = 20
    ["(2 + 3) * 4", fullCtx, 20, "parentheses override precedence"],
  ];

  let passed = 0;
  let failed = 0;

  for (const [expr, ctx, expected, label] of tests) {
    let result: number;
    let err: string | undefined;

    try {
      result = evaluateExpression(expr, ctx);
    } catch (e) {
      err = String(e);
      result = NaN;
    }

    if (err) {
      console.error(`FAIL [${label}]: threw error: ${err}`);
      failed++;
    } else if (result !== expected) {
      console.error(
        `FAIL [${label}]: evaluateExpression(${JSON.stringify(expr)}) → ${result}, expected ${expected}`,
      );
      failed++;
    } else {
      console.log(`PASS [${label}]: ${JSON.stringify(expr)} → ${result}`);
      passed++;
    }
  }

  console.log(`\n${passed} passed, ${failed} failed out of ${tests.length} tests`);

  if (failed > 0) {
    throw new Error(`Expression evaluator: ${failed} test(s) failed`);
  }
}
